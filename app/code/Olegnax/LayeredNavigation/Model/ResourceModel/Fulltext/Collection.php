...
class Collection extends \Magento\Catalog\Model\ResourceModel\Product\Collection
{
    ...
    public function getMemRequestBuilder()
    {
        if ($this->memRequestBuilder === null) {
            ...
            if (is_null($priceRangeCalculation)) {
    ...
    public function addAttributeToSort($attribute, $dir = self::SORT_ORDER_ASC)
    {
        ...
        if ($attribute == 'product_discount' && $storeId != 0) {
            if ($this->_productLimitationFilters->isUsingPriceIndex()) {
                $this->getSelect()->order(
                    [
                        new \Zend_Db_Expr(' case when price_index.price =  price_index.final_price  THEN 0 
                    when price_index.price > price_index.final_price THEN 1 END  ' . "DESC"),
                        new \Zend_Db_Expr(' price_index.final_price ' . "$dir")
                    ]
                );
                return $this;
            }
